...
public class ProductArrayList implements Evaluation {
	...
	public int lookUpProductId(String item) {
		System.out.println("Looking up the product ID for: " + item);
		for (Product p : productList) {
			if (p.getName().equalsIgnoreCase(item))
				return p.getId();

		}
		System.out.println("No product or ID found.");
		return 0;
	...
	/*
	 * Logic to check if list is null: if(productList.isEmpty()) -exchange for
	 * listOfCustomers also return;
	 */
